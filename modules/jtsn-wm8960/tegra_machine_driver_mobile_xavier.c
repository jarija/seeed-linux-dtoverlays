				  ...
				  u64 formats)
{
	...
	if (rtd) {
		...
		dai_params->channels_min = channels;
		dai_params->formats = formats;
                pr_err("pcm runtime seeed-voicedard-2mic was OK");
	}
        else {
                pr_err("failed to find pcm runtime seeed-voicedard-2mic");
        }
...
static int tegra_machine_seeed_voice_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (err) {
		...
		return err;
                ...
                pr_err("setting wm8960/seeed-voicecard-2mic sysclk was ok!\n");
        }
	return 0;
}
...
static int codec_init(struct tegra_machine *machine)
{
	...
	for (i = 0; i < num_links; i++) {
		...
		else if (strstr(dai_links[i].name, "seeed-voicecard-2mic")) {
